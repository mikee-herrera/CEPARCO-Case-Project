...
class RiscVGUI:
    ...
    def step_execution(self):
        ...
        # Correct order: EX -> MEM -> WB -> Advance pipeline registers -> record snapshot
        # 1) EX stage (compute using current ID/EX)
        ex_mem_new = self.execute_current_instruction()
        branch_taken = ex_mem_new.get('cond', 0) == 1

        # 2) MEM stage: operate on current EX_MEM (from previous cycle)
        # memory_access reads self.pipeline_state['EX_MEM'] and writes to self.pipeline_state['MEM_WB']
        self.memory_access()

        # 3) WB stage: write using current MEM_WB
        self.write_back()

        # 4) Advance pipeline registers (EX_MEM <= ex_mem_new, ID_EX <= IF_ID, fetch if not branch freeze)
        self.pipeline_advance(ex_mem_new, branch_taken)

        # 5) Record snapshot for pipeline table (after advancement to capture state of registers)
    ...
    def record_pipeline_snapshot(self):
        ...
        if memwb_ir:
            ...
            if rd != 0:
                wb_rd_str = f"x{rd}=0x{REGISTER_FILE.get(rd,0):08x}"
            else:
                wb_rd_str = ""
    ...
    def _stage_summary(self, stage):
        # fallback summary (not used in new table)
        ir = stage.get('IR', 0)
        if ir:
            return f"0x{ir:08x}"
        parts = []
    ...
    def write_back(self):
        """WB stage: Write results to register file using MEM_WB"""
        ...
        if rd != 0:
            if opcode == "0110011":  # R-type
                val = mem.get('ALUOUTPUT', 0) & 0xFFFFFFFF
                REGISTER_FILE[rd] = val
    ...
    def pipeline_advance(self, ex_mem_new=None, branch_taken=False):
        ...
        Important: memory_access() and write_back() must already have been called for the old EX_MEM/MEM_WB.
        This function must NOT overwrite MEM_WB produced by memory_access().
        """
        ...
        # - insert bubble to ID/EX (so next EX is NOP)
        # - DO NOT fetch a new instruction this cycle (we will keep IF/ID unchanged or clear it)
        if branch_taken:
            # Insert bubble into ID_EX
            self.pipeline_state['ID_EX'] = {'A': 0, 'B': 0, 'IMM': 0, 'IR': 0, 'NPC': 0}
            # When branch is taken we do not fetch a new instruction: keep IF_ID as-is or clear to 0?
            # We'll clear IF_ID to 0 because the instruction that was in IF/ID is invalidated (spec depends).
            # To reflect a classic freeze (stall one cycle), we'll keep IF/ID as-is (so ID will see it next cycle).
            # In our approach: DO NOT fetch new instruction (so PC already updated by EX) - keep IF/ID unchanged.
            return
    ...
    def instruction_fetch(self):
        """IF stage: Fetch instruction from program memory into IF/ID"""
        pc = self.pipeline_state['PC']
        ...
        if pc in self.program_memory:
            ...
            # increment PC to next by default
    ...
    def execute_current_instruction(self):
        """EX stage: Execute instruction and compute results based on ID_EX"""
        ...
        # I-type instructions (ORI)
            ...
            if funct3 == "110":  # ORI
                # immediate is signed as stored; but ORI treats as zero-extended normally
        ...
        # Branch instructions
            ...
            if branch_taken:
                # Use ID/EX.NPC + IMM (IMM is in bytes as encoded by assembler)
                npc = idex.get('NPC', 0)
                branch_target = (npc + imm_val) & 0xFFFFFFFF
    ...
    def run_program(self):
        ...
        # Prime the pipeline by fetching the first instruction
        self.instruction_fetch()
        # Set ID/EX values for the first cycle if IF/ID contains an instruction
